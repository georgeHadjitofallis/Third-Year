...
public class KinectGestures
{
	...
	private const int leftFoot = (int)KinectWrapper.SkeletonJoint.LEFT_FOOT;
	private const int leftAnkle =(int)KinectWrapper.SkeletonJoint.LEFT_ANKLE;
	private const int rightFoot = (int)KinectWrapper.SkeletonJoint.RIGHT_FOOT;
	private const int rightAnkle = (int)KinectWrapper.SkeletonJoint.RIGHT_ANKLE; 
	private const int rightKnee = (int)KinectWrapper.SkeletonJoint.RIGHT_KNEE;
	private const int leftKnee =(int)KinectWrapper.SkeletonJoint.LEFT_KNEE;
...
//						else 
		...
		case Gestures.Walk:
			switch(gestureData.state)//This Works
			{
			case 0:  // gesture detection
				
				if(jointsTracked[rightFoot] && jointsTracked[leftFoot] && jointsTracked[rightHandIndex] && jointsTracked[leftHandIndex] && jointsTracked[hipCenterIndex] &&
				   (jointsPos[rightHandIndex].z < jointsPos[hipCenterIndex].z || jointsPos[leftHandIndex].z < jointsPos[hipCenterIndex].z) &&
				   (Mathf.Abs(jointsPos[rightFoot].y - jointsPos[leftFoot].y)) > 0.2f &&
				   jointsTracked[rightAnkle] && jointsTracked[leftAnkle] &&
				   (Mathf.Abs(jointsPos[rightAnkle].y - jointsPos[leftAnkle].y)) > 0.2f)
				{
					//Debug.Log("Srart");
					SetGestureJoint(ref gestureData, timestamp, rightFoot, jointsPos[rightFoot]);
					//gestureData.state++;
				}
				/*else{
					Debug.Log("");
					Debug.Log("Srart" + jointsTracked[rightFoot]);
					Debug.Log("Srart" + jointsTracked[leftFoot]);
					Debug.Log("Srart" + (Mathf.Abs(jointsPos[rightFoot].y - jointsPos[leftFoot].y) > 0.3f));
					Debug.Log("Srart" + jointsTracked[rightAnkle] );
					Debug.Log("Srart" +  jointsTracked[leftAnkle]);
					Debug.Log("Srart" + (Mathf.Abs(jointsPos[rightAnkle].y - jointsPos[leftAnkle].y) > 0.3f));
				}*/
				//Debug.Log(""+ gestureData.state);
				break;
				
			case 1:  // gesture complete
				//Debug.Log("Case Cont.");
				bool isInPose = jointsTracked[rightFoot] && jointsTracked[leftFoot] &&
					(Mathf.Abs(jointsPos[rightFoot].y - jointsPos[leftFoot].y)) < 0.2f &&
						jointsTracked[rightAnkle] && jointsTracked[leftAnkle] &&
						(Mathf.Abs(jointsPos[rightAnkle].y - jointsPos[leftAnkle].y)) < 0.2f;
				Vector3 jointPos = jointsPos[gestureData.joint];
		...
		case Gestures.WalkBack:
			switch(gestureData.state)//This Works
			{
			case 0:  // gesture detection
				//Debug.Log("Hand" + jointsPos[rightHandIndex]);
				if(jointsTracked[rightFoot] && jointsTracked[leftFoot] && jointsTracked[rightHandIndex] && jointsTracked[leftHandIndex] && jointsTracked[hipCenterIndex] &&
				   (jointsPos[rightHandIndex].z > jointsPos[hipCenterIndex].z && jointsPos[leftHandIndex].z > jointsPos[hipCenterIndex].z) &&
				   (Mathf.Abs(jointsPos[rightFoot].y - jointsPos[leftFoot].y)) > 0.2f &&
				   jointsTracked[rightAnkle] && jointsTracked[leftAnkle] &&
		...
		case Gestures.RightShoot:
			switch(gestureData.state)
			{
			case 0:  // gesture detection - phase 1
				Debug.Log("A: "+ jointsPos[rightHandIndex].y + "- " +jointsPos[rightElbowIndex].y);
				Debug.Log("B: "+jointsPos[rightElbowIndex].y+" - " + jointsPos[rightShoulderIndex].y) ;
				Debug.Log("g: "+ jointsPos[rightHandIndex].x + " - " + jointsPos[rightElbowIndex].x);
				Debug.Log("d: "+jointsPos[rightHandIndex].z + " -"+ jointsPos[rightElbowIndex].z);
				if(jointsTracked[rightHandIndex] && jointsTracked[rightElbowIndex] && jointsTracked[rightShoulderIndex] && 
				   Mathf.Abs(jointsPos[rightHandIndex].y - jointsPos[rightElbowIndex].y) < 0.08f &&
				   Mathf.Abs(jointsPos[rightElbowIndex].y - jointsPos[rightShoulderIndex].y) < 0.08f &&
				   (Mathf.Abs(jointsPos[rightHandIndex].x - jointsPos[rightElbowIndex].x) > 0.15f ||
				 Mathf.Abs(jointsPos[rightHandIndex].z - jointsPos[rightElbowIndex].z) > 0.15f))
				{
					Debug.Log("state 0 £" +gestureData.state);
					//SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex]);
					gestureData.state++;
					Debug.Log(gestureData.state);
				}
			...
			case 1:  // gesture phase 2 = complete
				Debug.Log("nnextstate");
				//if((timestamp - gestureData.timestamp) < 1.5f)
				//{
					bool isInPose = jointsTracked[rightHandIndex] && jointsTracked[rightElbowIndex] && jointsTracked[rightShoulderIndex] && 
				   Mathf.Abs(jointsPos[rightHandIndex].y - jointsPos[rightElbowIndex].y) < 0.05f &&
				   Mathf.Abs(jointsPos[rightElbowIndex].y - jointsPos[rightShoulderIndex].y) < 0.05f &&
				   (Mathf.Abs(jointsPos[rightHandIndex].x - jointsPos[rightElbowIndex].x) > 0.15f ||
							 Mathf.Abs(jointsPos[rightHandIndex].z - jointsPos[rightElbowIndex].z) > 0.15f);
					Debug.Log(
						"n case1 s n pose : " + isInPose);
					if(isInPose)
					{
						Debug.Log("cHarched state 1" );
						Vector3 jointPos = jointsPos[gestureData.joint];
						CheckPoseComplete(ref gestureData, timestamp, jointPos, isInPose, 0f);
					}
				//}
