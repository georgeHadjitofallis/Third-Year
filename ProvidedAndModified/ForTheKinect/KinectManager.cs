...
public class KinectManager : MonoBehaviour
{
	...
	protected GestureEffects theGestureEffect = null;
...
//				break;
	...
	void Start()
	{
		...
		PlayerController = transform.root.GetComponentInChildren<OVRPlayerController> ();
		try
		{
...
//				usersMapRect.x -= cameraRect.width * DisplayMapsWidthPercent; //usersClrTex.width / 2;
	...
	void Update()
	{
		theGestureEffect = transform.root.GetComponentInChildren<GestureEffects> ();
		if(KinectInitialized)
		{
			...
			if(Player1Calibrated)
			{
				...
				foreach(KinectGestures.GestureData gestureData in player1Gestures)
				{
					if(gestureData.complete)
					{
						...
						else if(gestureData.gesture == KinectGestures.Gestures.SwipeLeft)
						{
							//Debug.Log("SwipeLeft" + transform.root.ToString());
							//transform.root.transform.Rotate(0,-30,0);
							theGestureEffect.SwipeLeftEffect(PlayerController,transform.root);
							//PlayerController.RotateCamerasFromGestrure(-30);
							ResetGesture(Player1ID, gestureData.gesture);
						}
						else if(gestureData.gesture == KinectGestures.Gestures.SwipeRight)
						{
							//Debug.Log("SwipeRight"+ transform.root.ToString());
							//transform.root.transform.Rotate(0,30,0);
							theGestureEffect.SwipeRightEffect(PlayerController, transform.root);
							//PlayerController.RotateCamerasFromGestrure(30);
							ResetGesture(Player1ID, gestureData.gesture);
						}
						else if(gestureData.gesture == KinectGestures.Gestures.Walk)
						{
							//Debug.Log("Push" + transform.root.rotation.ToString());
							Vector3 speed =transform.rotation * new Vector3( 0, 0, 50);
							theGestureEffect.WalkEffect(Controller,speed);
							//Controller.Move(speed * Time.deltaTime);
							ResetGesture(Player1ID, gestureData.gesture);
						}
						else if(gestureData.gesture == KinectGestures.Gestures.WalkBack)
						{
							//Debug.Log("Push" + transform.root.rotation.ToString());
							Vector3 speed =transform.rotation * new Vector3( 0, 0, -20);
							theGestureEffect.WalkBackEffect(Controller,speed);
							//Controller.Move(speed * Time.deltaTime);
							ResetGesture(Player1ID, gestureData.gesture);
						}
						else if(gestureData.gesture == KinectGestures.Gestures.Push)
						{
							//Debug.Log("Push" + transform.root.ToString());
							theGestureEffect.PushEffect();
							ResetGesture(Player1ID, gestureData.gesture);
						}
						else if(gestureData.gesture == KinectGestures.Gestures.RightShoot)
						{
							Debug.Log("Iddit");
							if(!audio.isPlaying){
